...
/*   Updated: 2021/04/24 11:46:39 by tpouget          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_matrix3x3		transpose_matrix(t_matrix3x3 matrix)
{
	t_matrix3x3	result;
...
t_vec3			apply_rotation_to_ray(t_vec3 ray, t_matrix3x3 rot_matrix)
{
	...
	transposed = transpose_matrix(rot_matrix);
	result.x = dot(transposed.right, ray);
	result.y = dot(transposed.up, ray);
	result.z = dot(transposed.forward, ray);
	return (result);
}
